...
import prisma from '../src/utils/prisma'; // Updated path
import { Role } from '@prisma/client'; // Asegúrate que Role está exportado o definido
    ...
    try {
        ...
        const newUser = await prisma.user.create({
            data: {
                ...
                name,
                role: role as Role, // Asegúrate que 'role' es uno de los valores del enum Role
        ...
        const token = jwt.sign(
            { id: newUser.id, role: newUser.role }, // <--- CAMBIA 'userId' a 'id'
