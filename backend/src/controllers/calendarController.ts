...
export const initiateGoogleAuth = async (req: RequestWithUser, res: Response) => {
  try {
    ...
    res.json({ success: true, authUrl, message: 'Redirige al usuario a esta URL para autorizar el acceso' });
  } catch (error) {
...
export const getCalendarIntegrationStatus = async (req: RequestWithUser, res: Response) => {
  try {
    ...
    if (!integration) {
      return res.json({ hasIntegration: false, provider: null, email: null });
    }
...
export const disconnectCalendarIntegration = async (req: RequestWithUser, res: Response) => {
  try {
    ...
    res.json({ success: true, message: `Integración con ${provider} desconectada exitosamente` });
  } catch (error) {
