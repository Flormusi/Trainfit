...
export const createOrUpdateProfile = async (req: Request, res: Response): Promise<void> => {
  console.log('🚀 createOrUpdateProfile controller HIT');
  try {
    const { name, phone, goals, weight, medicalConditions, medications, injuries, trainingDaysPerWeek, initialObjective } = req.body;
    ...
    // Prepare name for creation case, fallback to user name or default
    const nameForCreate = name || req.user?.name || 'Usuario';

    console.log('🧾 Datos recibidos para guardar perfil:', {
      name, phone, goals, weight, medicalConditions, medications, injuries, trainingDaysPerWeek, initialObjective
    });
    ...
    const updateData: any = {
        ...
        medications,
        injuries,
        trainingDaysPerWeek: trainingDaysPerWeek ? parseInt(trainingDaysPerWeek.toString()) : undefined,
        initialObjective
    };
