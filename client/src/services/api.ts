...
export const trainerApi = {
      ...
      if (response && response.data) {
        return {
          data: {
            ...
            routineCount: response.data.routineCount || 0,
            exerciseCount: response.data.exerciseCount || 0
          }
        };
      } else {
  ...
  getRoutineSchedule: async (): Promise<{ success: boolean; data: any[] }> => {
  ...
  updateNutritionPlan: async (planId: string, planData: any) => {
